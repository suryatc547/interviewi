...
## 3. Route (`interviewi-api/controllers/interview_controller.py`)
...
- Folder/file naming: kebab-case under `src/app/components/<name>/` with `name.component.{ts,css,html}`; the service file matches its class (`interview.service.ts` ↔ `InterviewService`, folder `interview-form/` ↔ `InterviewFormComponent`). Preserve the current naming, don't introduce `qc-*`/`*-controller` variants.

## 6. Verification

Backend: add/adjust **pytest tests** in `interviewi-api/tests/` (see `test_api.py` — Flask test client with the `stub_llm` fixture, `test_guardrails.py` for pure functions). Run `python -m pytest -q` from `interviewi-api/` and `python -m ruff check .`. Frontend: add a service spec (`interview.service.spec.ts`, `HttpClientTestingModule`) and/or a component smoke test; run `npm run test:ci`, `npm run lint`, `npm run build` from `web/`.
