...
**AI Interview Wizard (interviewi)** — a multi-page technical interview simulator powered by Google Gemini. A user fills in their tech stack/experience, the backend generates interview questions, the user answers them one at a time in a wizard, and answers are scored by AI in the background.
...
| --- | --- | --- | --- |
| POST | `/generate` | Create user+interview, generate questions | 400 missing fields; **502 + interview deleted if generation returns no questions** |
...
- **Models** (`models/models.py`): `db` is a module-level `SQLAlchemy()` instance. Columns use `db.Column`. Timestamps use `datetime.utcnow` with `onupdate` for updated-at fields.
